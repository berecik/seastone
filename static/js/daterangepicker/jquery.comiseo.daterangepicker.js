...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildTriggerButton($originalElement, classnameContext, options) {
		...
		function init() {
			fixReferences();
			$self = $('<button type="button" class="fa fa-calendar pad10-r">&nbsp;&nbsp;<span class="calendar-label"></span></button>')
            ...
            $self.find('.calendar-label').text(options.initialText);
		}

		function getLabel() {
			return $self.find('.calendar-label').text();
		}

		function setLabel(value) {
			$self.find('.calendar-label').text(value);
		}
